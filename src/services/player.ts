...
/**
 * Helper to wrap Premium-only API calls and silently ignore 403 errors
 * (Free accounts will get 403 on playback controls)
 */
const premiumOnly = async <T>(fn: () => Promise<T>): Promise<T | undefined> => {
  try {
    return await fn();
  } catch (error: any) {
    if (error?.response?.status === 403) {
      console.warn('This feature requires Spotify Premium');
      return undefined;
    }
    throw error;
  }
...
const transferPlayback = async (deviceId: string) => {
  return premiumOnly(() => axios.put('/me/player', { device_ids: [deviceId] }));
};
...
const startPlayback = async (
  ...
  return premiumOnly(() => axios.put('/me/player/play', body));
};
...
const pausePlayback = async () => {
  return premiumOnly(() => axios.put('/me/player/pause'));
};
...
const nextTrack = async () => {
  return premiumOnly(() => axios.post('/me/player/next'));
};
...
const previousTrack = async () => {
  return premiumOnly(() => axios.post('/me/player/previous'));
};
...
const seekToPosition = async (position_ms: number) => {
  return premiumOnly(() => axios.put('/me/player/seek', {}, { params: { position_ms } }));
};
...
const setRepeatMode = async (state: 'track' | 'context' | 'off') => {
  return premiumOnly(() => axios.put('/me/player/repeat', {}, { params: { state } }));
};
...
const setVolume = async (volume_percent: number) => {
  return premiumOnly(() => axios.put('/me/player/volume', {}, { params: { volume_percent } }));
};
...
const toggleShuffle = async (state: boolean) => {
  return premiumOnly(() => axios.put('/me/player/shuffle', {}, { params: { state } }));
};
...
const addToQueue = async (uri: string) => {
  return premiumOnly(() => axios.post('/me/player/queue', {}, { params: { uri } }));
};
